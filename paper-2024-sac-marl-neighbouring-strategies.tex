...
In independent learner, each agent learns its policy independently, disregarding the actions and policies of other agents. 
...
\gianlu{
  todo -- discuss the main three solution, policy based (MAPPO, MADDPG, etc.), value based (QMIX, VDQN, etc.) and mean field (mean-q learning with field-informed RL).
}
...
\begin{algorithm}
  ...
  \ForEach{agent $i \in N$}{
      ...
      Initialize replay buffer $\mathcal{B}^i$
  }
  \For{episode = 1 to M}{
      \ForEach{agent $i \in N$}{
      ...
      \For{t = 0 to T}{
          \ForEach{agent $i \in N$}{
              ...
              Store $(o_t^i, a_t^i, r_{t+1}^i, o_{t+1}^i)$ in $\mathcal{B}^i$\\
              Sample a mini-batch from $\mathcal{B}^i$\\
